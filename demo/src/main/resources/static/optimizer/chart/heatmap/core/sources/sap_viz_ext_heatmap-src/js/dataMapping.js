define("sap_viz_ext_heatmap-src/js/dataMapping", ["sap_viz_ext_heatmap-src/js/utils/dynamicsUtil"], function(dynamicsUtil) {
	"use strict";
	var processData = function(data, feeds, done) {
		// Build name index so that dimension/measure sets can be accessed by name
		dynamicsUtil.buildNameIdx(feeds);
		/*
		...
		dynamicsUtil.toTable(data, mapper, function(err, pData) {
			if (err) {
