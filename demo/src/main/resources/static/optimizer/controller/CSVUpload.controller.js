sap.ui.define([
		...
		"use strict";
	var pageNo = 0;
	return BaseController.extend("dhi.optimizer.controller.CSVUpload", {
		/**
		 * alarm & event init
		 */
		onInit : function () {
		...
		fnSearchCallback: function(oModel){
			...
			if(rtnData.code==200){
				...
				if(bindingData.length>0){
					...
					var regExp = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi;
					jQuery.sap.log.info(bindingData)
//					var dateTime, ndate;
...
//					}
		...
		rowScrolling: function(oEvent) {
			...
			if(lastDataLength>0){
				if(lastRowIndex==lastDataLength){
					pageNo++;
					oTable.setBusy(true);
					this.oParam.url = this.getApiUrl()+"/alarm_event/data/";
					this.oParam.data.page_no = pageNo;
		...
		createColumnConfig: function() {
			return [
				{ label: 'DATETIME', 		property: 'datetime', 			textAlign: "center", type: 'DateTime',	width: 15 },
